...
function Notes() {
    ...
    function handleEdit(note) {
        if (note.password) {
          onOpenPassword(note, () => navigate(`/edit/${note.id}`))
        } else {
    ...
    function handleDownload(note) {
        if (note.password) {
          onOpenPassword(note, () => onOpenDownload(note))
        } else {
          onOpenDownload(note)
        }
    }
    ...
    function handleDelete(note) {
        if (note.password) {
          onOpenPassword(note, () => onOpenDelete(note))
        } else {
          onOpenDelete(note)
        }
    }
    ...
    function handleOpenNote(note) {
        if (note.password) {
          onOpenPassword(note, () => navigate(`/note/${note.id}`))
        } else {
    ...
    return (
        <div className={styles.notes} data-lang={language}>
            ...
            {filteredNotes.map(note => (
                <div key={note.id} to={`/note/${note.id}`} className={styles.notesItem}
                onClick={(e) => {
                    ...
                    <div className={styles.notesItemContainer}>
                        ...
                        <button onClick={() => handleDelete(note)} className={isDeleteActive && noteId === note.id ? styles.notesItemContainerActive : ''}>
                            <svg width="17" height="20" viewBox="0 0 17 20" fill="none" xmlns="http://www.w3.org/2000/svg">
